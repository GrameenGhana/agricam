    ...
    public function readImageDirectory($directory) {
        ...
        if (!$galleryArray) {
            ...
            foreach ($dirArray as $key => $image) {
                ...
                $idx = htmlentities(pathinfo($image['real_path'], PATHINFO_BASENAME));

                $galleryArray['images'][$idx] = array(
                ...
                $galleryArray['images'][$idx]['file_title_plain'] = preg_replace("/(CAM\d\s*)|IMG\s/","", $galleryArray['images'][$idx]['file_title']);
                #print '<pre>'; print_r($galleryArray); print '</pre>'; exit;
            }
